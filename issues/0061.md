...
## Resolution — fix (2026-06-24, commit `d3f2ced`)

Regenerated `favicon.png` (32×32), `apple-touch-icon.png` (180×180), and a new `favicon-16.png` (16×16) from `design/ShortLinks.png` with **Pillow `LANCZOS`** (replacing the `sips` downscale that caused the ringing). Added a real multi-size `favicon.ico` (16/32/48) and referenced all from `web/index.html` with `sizes=` hints. **Verified at the pixel level**: enlarged the 32px output with NEAREST and compared LANCZOS/HAMMING/BILINEAR/BOX — all clean, none show the sips white-line artifacts. The new `favicon.ico` also fixes the earlier `/favicon.ico → text/html` defect: booted the dev server (`STORAGE=json`) and confirmed `/favicon.ico → 200 image/x-icon`, `/favicon.png → 200 image/png`, `/favicon-16.png → 200 image/png`. (No subagent dispatch — done directly with verification, so no work-log row.) Final tab-icon look is the user's eyeball.

## Files changed (fix)

- `web/public/favicon.png`, `apple-touch-icon.png` — regenerated (LANCZOS).
- `web/public/favicon-16.png`, `web/public/favicon.ico` — NEW.
- `web/index.html` — `sizes=` hints + `favicon.ico` link.
